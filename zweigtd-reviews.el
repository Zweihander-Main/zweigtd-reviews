...
(defvar zweigtd-reviews-goals-file nil
  "")

(defvar zweigtd-reviews-non-goals-string "OTHER"
  "")

(defvar zweigtd-reviews-goals-list nil
  "")
  ...
  (if (string= goal zweigtd-reviews-non-goals-string)
    ...
    (let* ((headline
            (plist-get
             (car
              (org-ql-query
                :from zweigtd-reviews-goals-file
                :where `(and (todo "TODO")
                             (parent ,goal))))
             'headline))
...
Will return non-goal todo num if goal is \"OTHER\"."
  (let ((match
         (if (string= goal zweigtd-reviews-non-goals-string)
             `(not (tags ,@zweigtd-reviews-goals-list) ;; TODO potential bug
           `(tags ,goal))))
    (length
     (org-ql-query
...
To be used in org-capture-template as the position function."
  ...
  (let ((marker (org-find-olp
                 (cons
                  (org-capture-expand-file zweigtd-reviews-goals-file)
    ...
    (org-datetree-find-month-create zweigtd-reviews--current-working-date 'subtree-at-point)))
