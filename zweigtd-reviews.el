...
(defvar zweigtd-reviews--current-working-date '(1 1 1900)
  "Current date being used by `org-capture' for generating reviews.
Usually generated by calendar-read-date ie Gregorian format.
...
To be used in org-capture-template as the position function."
  ...
  (let ((marker (org-find-olp
                 (cons
                  (org-capture-expand-file zweigtd-reviews-file) ;; TODO reviews file
...
;;;###autoload
(defun zweigtd-reviews-genreview (interval grouping &optional append num-completed priority only-goals no-task-headings)
...
NO-TASK-HEADINGS will not print the actual tasks closed."
  (let* ((ts-cons (zweigtd-reviews--query-interval interval)) ;; TODO listp
